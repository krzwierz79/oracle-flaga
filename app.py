...
app.config[
    "UPLOAD_FOLDER"
] = "static"  # not used right now (possibly automate some uploads later)
...
def gaps():
    ...
    return render_template("gaps.html", box=box, passage=passage)
...
def flaga():
    ...
    # use a rangom flag
    ta_flaga = os.path.join(
        app.config["UPLOAD_FOLDER"], "flag_image/Flaga__{}.jpg".format(xd)
    )
...
def gather_heroes(xd):
    ...
    if (xd % 2) == 0:
        heroes = pirates  # even num in static/flaga_image/Flaga__
        hero_tag = "pirate"
    else:
        heroes = patriots  # odd
        hero_tag = "patriot"
    ...
    for hero in heroes:
        if hero not in saved_heroes:
            ...
            with open(
                "dane/heroes/saved_heroes/" + hero + "." + hero_tag,
                "w+",
                encoding="utf-8",
            ) as f:
                f.write(hero + "\n")
        ...
        for hero_file in os.listdir("dane/heroes/saved_heroes"):
            ...
            if hero_file.endswith(hero_tag):
                ...
                hero = {}
                with open(
                    f"dane/heroes/saved_heroes/{hero_file}", encoding="utf-8"
                ) as f:
...
def hero_think(name):
    ...
    with open("dane/heroes/hero_think/" + name + ".quote", "w+", encoding="utf-8") as f:
        ...
        for line in hero_wikiquotes.text.split("\n"):
            ...
            if line.startswith("<ul><li>"):
                ...
                # more strings to drop
                    ...
                    not quote.startswith("Opis")
                    and not quote.startswith("Autor")
                    and not quote.startswith("Źródło")
                ):
                    f.write(quote + "\n")
                    print("-", quote)
